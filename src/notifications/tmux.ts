...
import { getMultiplexerProvider } from "../winmux/provider/select-provider.js";
...
function execTmux(args: string[]): string {
  // Route through MultiplexerProvider so native Windows talks to the
  // omx-winmux daemon instead of looking up a tmux binary that doesn't exist.
  if (process.platform === "win32") {
    const result = getMultiplexerProvider().run(args);
  ...
  return execFileSync(resolveTmuxBinaryForPlatform() || "tmux", args, {
    ...
    // win32 already returned above; this branch is POSIX-only.
    windowsHide: false,
  }).trim();
}
...
export function captureTmuxPaneWithLiveness(paneId?: string | null, lines: number = 12): TmuxPaneCaptureResult {
  ...
  if (!process.env.TMUX && !paneId && process.platform !== "win32") {
    // On non-Windows TMUX env presence is the cheap liveness check; on
    // Windows the omx-winmux daemon owns liveness, so skip this guard.
  ...
  try {
    ...
    try {
      ...
      // EPERM on Windows means the process exists but is in a different
      // session; that's still "live" for our purposes.
      const code = (err as NodeJS.ErrnoException).code;
      if (code !== "EPERM") return { content: null, live: false };
    }

    // Route capture through the multiplexer abstraction so winmux can serve it.
    if (process.platform === "win32") {
      const result = getMultiplexerProvider().run(buildCapturePaneArgv(target, clampedLines));
      if (!result.ok) return { content: null, live: false };
      const trimmed = result.stdout.trim();
      return { content: trimmed || null, live: true };
    }
