...
export function capturePaneContent(paneId: string, lines: number = 15): string {
  if (process.platform === 'win32') {
    const result = getMultiplexerProvider().run(buildCapturePaneArgv(paneId, lines));
...
export function sendToPane(
  ...
  // On native Windows route through the multiplexer abstraction so winmux
  // forwards key sequences into the daemon-owned PTY instead of looking for
  // a non-existent tmux binary.
  if (process.platform === 'win32') {
    const provider = getMultiplexerProvider();
    for (const [index, argv] of argvs.entries()) {
      const result = provider.run(['send-keys', ...argv]);
