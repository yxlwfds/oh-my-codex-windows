...
describe('owx sparkshell', () => {
  it('includes sparkshell in top-level help output', async () => {
    ...
    try {
      ...
      assert.match(result.stdout, /(?:omx|owx) explore\s+Default read-only exploration entrypoint \(may adaptively use sparkshell backend\)/);
      assert.match(result.stdout, /(?:omx|owx) sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /(?:omx|owx) sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
  ...
  it('prints sparkshell usage when invoked with --help', async () => {
    ...
    try {
      ...
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage: (?:omx|owx) sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /or: (?:omx|owx) sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
