...
function runTmux(args: string[]): { ok: true; stdout: string } | { ok: false; stderr: string } {
  // Multiplexer abstraction: on non-Windows platforms this routes to the real
  // tmux binary via `TmuxProvider` (zero behaviour change). On native Windows
  // it routes to `WinMuxProvider` which talks to the omx-winmux daemon.
  return getMultiplexerProvider().run(args);
}
...
// Check if tmux (or its Windows equivalent, omx-winmux) is available.
// On native Windows we delegate to `WinMuxProvider.isAvailable()` which pings
// the omx-winmux daemon. There is intentionally NO fallback to psmux/tmux —
// see plan §1/§4.2.
export function isTmuxAvailable(): boolean {
  return getMultiplexerProvider().isAvailable();
}
