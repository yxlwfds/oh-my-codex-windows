...
- Prefer `owx explore --prompt ...` with narrow, concrete lookup goals; if the ask is broad, multi-part, or needs synthesis beyond simple repository inspection, report the limitation so the caller can fall back to the richer normal path.
- Prefer `owx explore --prompt ...` for short one-off asks and `owx explore --prompt-file ...` when the brief is longer or reusable.
