...
- When active guidance enables `USE_OMX_EXPLORE_CMD`, use `owx explore` FIRST for simple read-only file/symbol/pattern lookups; use `owx sparkshell` for noisy read-only verification summaries; fall back normally if either is insufficient.
</ask_gate>
