...
1. **外键与关联策略需统一约定**：
   - 表之间是否创建物理外键（FOREIGN KEY），以及哪些关联放在数据库层 / 应用层处理，必须由本项目架构/DBA 在统一的数据库规范中明确约定，并在本技能相关的迁移脚本中保持一致。下面示例默认采用在应用层处理关联的微服务建模方式，但可按项目需要调整。
2. **实体承袭**：
...
- [ ] 新表的外键/关联是否符合项目统一约定的数据库建模规范？（例如：在 Java 逻辑层通过 Dubbo 处理关联，或按架构组约定使用数据库外键。）
- [ ] 实体类是否继承了 BaseEntity ?
