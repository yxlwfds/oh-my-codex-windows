...
- When the code-intelligence toolchain is available, default to Windsurf's built-in Fast Context (LSP/grep) for quick codebase traversal and symbol lookup first. Upgrade to `ast-grep` for syntax-aware rewrite/structural search when built-in methods are insufficient；避免在全局约定中强推额外的语义搜索二进制工具。
- Code search workflow:
  - Start with built-in Fast Context (LSP/grep) to resolve explicit symbols, exact file paths, or short queries.
  - For abstract or natural-language behavior/intent queries, first refine Fast Context queries or switch to `ast-grep` for syntax-aware patterns；如仍明显不足，再参考本仓文档中约定的其他语义检索工具（如果存在）。
  - Inspect full files only when the returned chunk is not enough context。
  - Optionally use any project-local semantic search utilities that are explicitly documented in this repo; do not assume a specific binary in global guidance。
...
- Auto-activate runtime workflows only when the current session is actually running under OMX CLI/runtime (for example, launched via `omx`, with OMX session overlay/runtime state available, or when the user explicitly asks to run `owx ...` in the shell).
...
Command routing:
- When `USE_OMX_EXPLORE_CMD` enables advisory routing, strongly prefer `owx explore` as the default surface for simple read-only repository lookup tasks (files, symbols, patterns, relationships).
- For simple file/symbol lookups, use `owx explore` FIRST before attempting full code analysis.

Use `owx explore --prompt ...` for simple read-only lookups through the shell-only, allowlisted, read-only path. Use `owx sparkshell` for noisy read-only shell commands, bounded verification, repo-wide listing/search, or explicit `owx sparkshell --tmux-pane` summaries. Treat sparkshell as explicit opt-in. When to use what: keep ambiguous, implementation-heavy, edit-heavy, diagnostics, tests, MCP/web, and complex shell work on the normal path; if `owx explore` or `owx sparkshell` is incomplete, retry narrower or gracefully fall back to the normal path.
