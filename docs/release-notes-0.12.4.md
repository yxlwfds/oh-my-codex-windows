...
- **MCP-CLI parity** — `owx state`, `owx notepad`, `owx project-memory`, `owx trace`, and `owx code-intel` expose MCP server tools through the CLI, making every MCP operation scriptable without transport overhead.
...
### Added
- MCP-CLI parity surface: `owx state`, `owx notepad`, `owx project-memory`, `owx trace`, `owx code-intel`
...
- User-owned Codex hooks preserved during `owx setup` refresh
...
### Changed
- State CLI routed consistently through `owx state`
