...
`0.16.1` is a broad post-`0.16.0` hardening release. It improves `owx explore` safety and bounded execution, session-scoped runtime authority, Team approved-handoff repair paths, Codex goal-mode durability, plugin/cache behavior, native launch portability, CI reliability, and release-install proof.
...
- `owx explore` local fast-path file reads no longer follow symlinks; symlinked paths fall back to the harness path.
