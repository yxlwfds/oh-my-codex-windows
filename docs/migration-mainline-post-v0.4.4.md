...
- use `owx setup` and need predictable install scope behavior,
- run `owx team`/tmux workflows and want the latest reliability fixes,
...
- `owx setup` now supports scope-aware install modes (`user`, `project`). Legacy `project-local` values are auto-migrated.
...
| `$deepinit` | `owx agents-init [path]` | Lightweight CLI successor for AGENTS.md bootstrap only; immediate child directories only, unmanaged files preserved unless `--force`. |
...
- [ ] If using spark worker routing, verify flags are available:
  ```bash
  owx --help | rg "spark|madmax-spark"
  ```
