...
- Retired team MCP config repair: `owx doctor` and launch path realign retired entries on upgrade (PR [#1436](https://github.com/Yeachan-Heo/oh-my-codex/pull/1436))
...
- `owx explore` now emits a clear actionable error when cargo is a rustup shim with no default toolchain configured, instead of surfacing the raw rustup error. Users are directed to `rustup default stable`, `OMX_EXPLORE_BIN`, or `owx doctor`. (`src/cli/explore.ts`)
