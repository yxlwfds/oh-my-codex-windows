...
- **Nativer Harness für `owx explore`** — führt Read-only-Repository-Exploration über einen schnelleren und strengeren Rust-Pfad aus.
- **`owx sparkshell`** — native Operator-Oberfläche für Inspektion mit Zusammenfassungen langer Ausgaben und expliziter tmux-Pane-Erfassung.
...
```bash
owx                # Codex starten (+ HUD in tmux wenn verfügbar)
owx setup          # Prompts/Skills/Config nach Bereich installieren + Projekt-.omx + bereichsspezifische AGENTS.md
owx doctor         # Installations-/Laufzeitdiagnose
owx doctor --team  # Team/Swarm-Diagnose
owx team ...       # tmux-Team-Worker starten/Status/fortsetzen/herunterfahren
owx status         # Aktive Modi anzeigen
owx cancel         # Aktive Ausführungsmodi abbrechen
owx reasoning <mode> # low|medium|high|xhigh
owx tmux-hook ...  # init|status|validate|test
owx hooks ...      # init|status|validate|test (Plugin-Erweiterungs-Workflow)
...
OMX enthält jetzt `owx hooks` für Plugin-Gerüstbau und -Validierung.

- `owx tmux-hook` wird weiterhin unterstützt und ist unverändert.
- `owx hooks` ist additiv und ersetzt keine tmux-hook-Workflows.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 owx     # AGENTS.md-Injektion deaktivieren
