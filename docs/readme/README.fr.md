...
- **Harness natif pour `owx explore`** — exécute l’exploration read-only du dépôt via une voie Rust plus rapide et plus stricte.
- **`owx sparkshell`** — surface native orientée opérateur, avec résumés de sorties longues et capture explicite de panneaux tmux.
...
```bash
owx                # Lancer Codex (+ HUD dans tmux si disponible)
owx setup          # Installer prompts/skills/config par scope + .omx du projet + AGENTS.md propre au scope
owx doctor         # Diagnostics d'installation/exécution
owx doctor --team  # Diagnostics Team/Swarm
owx team ...       # Démarrer/statut/reprendre/arrêter les workers d'équipe tmux
owx status         # Afficher les modes actifs
owx cancel         # Annuler les modes d'exécution actifs
owx reasoning <mode> # low|medium|high|xhigh
owx tmux-hook ...  # init|status|validate|test
owx hooks ...      # init|status|validate|test (workflow d'extension de plugins)
...
OMX inclut désormais `owx hooks` pour l'échafaudage et la validation de plugins.

- `owx tmux-hook` reste supporté et inchangé.
- `owx hooks` est additif et ne remplace pas les workflows tmux-hook.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 owx     # désactiver l'injection AGENTS.md
