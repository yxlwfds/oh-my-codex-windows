...
1. Запустіть `owx setup`
2. Запустіть з `owx --madmax --high`
...
- `owx setup` встановлює промпти, навички, конфіг і структуру AGENTS
- `owx doctor` перевіряє встановлення, коли щось здається не так
- `owx hud --watch` — це інструмент моніторингу стану, а не основний робочий процес користувача

### Explore і sparkshell

- `owx explore --prompt "..."` призначений для пошуку в репозиторії лише для читання
- `owx sparkshell <command>` призначений для перевірки в терміналі та обмеженої валідації
...
- `owx setup` має увімкнути нативні хуки Codex (`[features].hooks = true`) у
...
- `owx tmux-hook` зарезервовано для поведінки командного рушія та усунення проблем із застарілим tmux
...
`owx team` потребує інструмента, сумісного з tmux:
