...
- **`owx explore` 原生 harness** —— 以 Rust 原生 harness 更快且更嚴格地執行唯讀儲存庫探索。
- **`owx sparkshell`** —— 面向操作員的原生檢查介面，支援長輸出摘要與 tmux pane 擷取。
...
```bash
owx                  # 啟動 Codex（可用時在 tmux 中附帶 HUD）
owx setup            # 依範圍安裝提示詞/技能/設定 + 專案 .omx + 範圍專屬 AGENTS.md
owx doctor           # 安裝/執行期診斷
owx doctor --team    # 團隊/群集診斷
owx ask ...          # 詢問本地供應商顧問（claude|gemini），結果寫入 .omx/artifacts/*
owx team ...         # 啟動/狀態/恢復/關閉團隊工作進程（預設為互動式 tmux）
owx status           # 顯示目前活動模式
owx cancel           # 取消活動中的執行模式
owx reasoning <mode> # low|medium|high|xhigh
owx tmux-hook ...    # init|status|validate|test
owx hooks ...        # init|status|validate|test（插件擴充工作流程）
...
```bash
owx ask claude "review this diff"
owx ask gemini "brainstorm alternatives"
owx ask claude --agent-prompt executor "implement feature X with tests"
owx ask gemini --agent-prompt=planner --prompt "draft a rollout plan"
# 底層供應商 CLI 說明中的旗標：
...
```bash
OMX_TEAM_WORKER_LAUNCH_MODE=prompt owx team 2:executor "task"
```

## Hooks 擴充（附加介面）

OMX 現已包含 `owx hooks`，用於插件鷹架建立與驗證。

- `owx tmux-hook` 持續受支援，行為不變。
- `owx hooks` 屬於附加功能，不會取代 tmux-hook 工作流程。
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 owx     # 停用 AGENTS.md 注入
...
請在 Team 工作完成後另外執行 `owx ralph ...`。舊的 linked-Ralph 團隊路徑已不再是建議或支援的標準路徑。
