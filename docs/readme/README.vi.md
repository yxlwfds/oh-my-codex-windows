...
1. Chạy `owx setup`
2. Khởi động với `owx --madmax --high`
...
- `owx setup` cài prompt, skill, config và scaffold AGENTS
- `owx doctor` kiểm tra cài đặt khi có vấn đề
- `owx hud --watch` theo dõi trạng thái, không phải workflow chính

### Explore và sparkshell

- `owx explore --prompt "..."` tìm kiếm chỉ đọc trong repository
- `owx sparkshell <command>` chạy lệnh shell có kiểm soát output
...
`owx team` cần backend tương thích tmux:
