...
- **Нативный harness для `owx explore`** — ускоряет и ужесточает read-only исследование репозитория через Rust-путь.
- **`owx sparkshell`** — нативная операторская поверхность для инспекции с краткими сводками длинного вывода и явным захватом tmux-pane.
...
```bash
owx                # Запустить Codex (+ HUD в tmux при наличии)
owx setup          # Установить промпты/навыки/конфиг по области + .omx проекта + AGENTS.md для выбранной области
owx doctor         # Диагностика установки/среды выполнения
owx doctor --team  # Диагностика Team/swarm
owx team ...       # Запуск/статус/возобновление/завершение рабочих tmux
owx status         # Показать активные режимы
owx cancel         # Отменить активные режимы выполнения
owx reasoning <mode> # low|medium|high|xhigh
owx tmux-hook ...  # init|status|validate|test
owx hooks ...      # init|status|validate|test (рабочий процесс расширений плагинов)
...
OMX теперь включает `owx hooks` для создания шаблонов плагинов и валидации.

- `owx tmux-hook` по-прежнему поддерживается и не изменён.
- `owx hooks` является дополнительным и не заменяет рабочие процессы tmux-hook.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 owx     # отключить внедрение AGENTS.md
