...
> incompatible state first via `owx state ...` or the `omx_state.*` MCP tools,
...
- `owx state clear --input '{"mode":"team"}' --json`
...
5. denial messages mention both `owx state` and `omx_state.*`
