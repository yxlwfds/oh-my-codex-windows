...
`owx team ralph ...` linked launch path anymore. Under the multi-state
...
- Prompt-side `$ralph` workflow activation is not equivalent to `owx ralph --prd ...`; it may seed Ralph mode state and routing context, but the PRD startup gate remains an explicit CLI-path contract.
