...
If you use `$team`/`owx team` heavily, this release makes the runtime more consistent by standardizing around **CLI-first interop**.
...
- If you previously relied on legacy `team_*` MCP workflows, migrate to:
  - `owx team api <operation> ...`
- For notification setup, prefer:
  - `owx configure-notifications` (or skill equivalent)
