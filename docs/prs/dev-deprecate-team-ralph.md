# PR Draft: Deprecate `owx team ralph` and keep team standalone

## Target branch
`dev`

## Title
Deprecate `owx team ralph` and remove linked team↔Ralph lifecycle machinery

## Summary
This PR removes the built-in linked `owx team ralph` workflow and restores a clean separation between `team` and `ralph`.

After this change:
- `owx team ...` / `$team ...` is the only supported team launch path
- `owx ralph ...` / `$ralph ...` remains available as a separate, explicit follow-up
- team no longer creates, syncs, or depends on linked Ralph state
- legacy `owx team ralph ...` usage now fails with a clear deprecation error instead of being tolerated silently
...
- advertised/generated `owx team ralph ...` launch hints
- linked-Ralph-specific tests and compatibility paths inside team runtime

### Updated
- team help/usage text now documents only `owx team [N:agent-type] "<task>"`
...
- follow-up planner and pipeline launch hints now generate plain `owx team ...`
- shutdown/resume/state flows now operate on standalone team semantics only

### Behavior change
- `owx team ralph ...` is now rejected with an explicit deprecation error:
  - use `owx team ...` for coordinated team execution
  - use `owx ralph ...` separately if a later persistent follow-up loop is still needed
...
## Impact
### Before
- `owx team ralph ...` had special runtime behavior
...
### After
- `owx team ...` runs coordinated team execution only
- team owns its own verification lanes and shutdown evidence
- `owx ralph ...` is separate and explicit
...
## Risks / notes
- This is an intentional CLI behavior break for users still invoking `owx team ralph ...`
