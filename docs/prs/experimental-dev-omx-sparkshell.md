...
## Summary
This PR rebases the `owx sparkshell` feature line onto `experimental/dev` and preserves the follow-on team-status inspection work built on top of it.

It adds the native Rust-backed `owx sparkshell <command> [args...]` flow, plus a long sequence of `owx team status` improvements that surface pane-aware inspection metadata and ready-to-run sparkshell commands for leader triage.

## What’s included
### `owx sparkshell`
- adds CLI dispatch/help for `owx sparkshell`
...
### `owx team status` inspection improvements
