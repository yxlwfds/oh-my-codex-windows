...
- **Launch behavior is explicit by default and operator-controllable:** the default launch path remains detached-tmux; use `owx --direct` or `OMX_LAUNCH_POLICY=direct` (or `=tmux`/`=detached-tmux`) to force the desired launch mode.
