...
- `owx --direct` and `OMX_LAUNCH_POLICY=direct|tmux|detached-tmux|auto` make leader launch policy explicit and operator-controllable.
...
- Default supported interactive launches remain detached-tmux managed. Use `owx --direct` or `OMX_LAUNCH_POLICY=direct` to bypass tmux/HUD management, or `OMX_LAUNCH_POLICY=tmux|detached-tmux` to force tmux-managed startup.
