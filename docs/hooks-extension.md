...
- `.codex/hooks.json` = native Codex hook registrations installed by `owx setup`
- `.omx/hooks/*.mjs` = OMX plugin hooks dispatched by runtime/native events
- `owx tmux-hook` / notify-hook / derived watcher = tmux/runtime fallback surfaces

`owx setup` treats `.codex/hooks.json` as a shared-ownership file: it refreshes only the OMX-managed
wrapper entries that invoke `dist/scripts/codex-native-hook.js` and preserves user hook entries in the
same file. `owx uninstall` removes only those OMX-managed wrappers and leaves `.codex/hooks.json` in
place when user hooks remain.

> Compatibility guarantee: `owx tmux-hook` remains fully supported and unchanged.
> The new `owx hooks` command group is additive and does **not** replace tmux-hook workflows.
...
- `owx tmux-hook` remains a CLI/runtime workflow, not `sdk.omx.tmuxHook.*`
