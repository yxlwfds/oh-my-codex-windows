...
| Direct `owx autoresearch` remains deprecated with goal-mode replacement path | `src/cli/autoresearch.ts`, `src/autoresearch/goal.ts`, `skills/autoresearch-goal/SKILL.md`, `plugins/oh-my-codex/skills/autoresearch-goal/SKILL.md` | Covered by clean `npm test` and targeted goal workflow suite | Verified locally |
