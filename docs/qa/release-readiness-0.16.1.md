...
| --- | --- | --- |
| `owx explore` explicit local file fast-path could follow repo symlinks outside the repository | Explicit fast-path file lookup now rejects symbolic links before `stat()`/read and falls back to the harness | `src/cli/explore.ts`, `src/cli/__tests__/explore.test.ts` |
