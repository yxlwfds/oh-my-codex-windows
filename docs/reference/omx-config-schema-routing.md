...
`owx setup --scope project` persists the project-scope choice in `./.omx/setup-scope.json`; `owx doctor` prints the resolved setup scope and the Codex home/config paths it is checking.
...
`readConfiguredEnvOverrides()` also passes through other non-empty string values from `env` for launch helpers such as `owx explore` and `owx sparkshell`. Treat those as advanced environment overrides, not a schema for per-role model routing.
...
Use `owx team status <team-name> --model-inspect` when you need inspect hints for a running team; the regular status path avoids spending model quota on summaries.
...
- `owx reasoning <low|medium|high|xhigh>`, which edits the active Codex `config.toml`.
- `owx --high` and `owx --xhigh`, which pass `-c model_reasoning_effort="high|xhigh"` to Codex launch.
...
OMX_TEAM_WORKER_LAUNCH_ARGS='-c model_reasoning_effort="low" --model gpt-5.3-codex-spark' \
  owx team 3:explore "map the config surfaces"
```
...
`owx doctor` reports the resolved setup scope, Codex home, config path, hook coverage, prompt/skill/agent availability, and selected prompt-routing status. This verifies install wiring and which config tree OMX is checking.

A green `owx doctor` is not proof that the active Codex profile can authenticate or run the selected model. For that, use the same shell/profile/project and run:
