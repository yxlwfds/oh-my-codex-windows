...
- **Native goal-mode workflows** — `owx ultragoal`, `owx performance-goal`, and `owx autoresearch-goal` now provide durable repo-native artifacts, explicit handoffs, validation ledgers, and completion reconciliation against fresh Codex `get_goal` snapshots.
...
- **Autoresearch migration path** — direct `owx autoresearch` remains hard-deprecated; use `$autoresearch` or `owx autoresearch-goal` for goal-mode-backed research workflows.
